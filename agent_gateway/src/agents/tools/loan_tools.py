"""
Tools for interacting with user loan records via the MCP server.
"""
import sys
import os
import json
...
@tool
async def get_my_loans_tool(config: RunnableConfig) -> str:
    """
    ...
    try:
        ...
        async with sse_client(MCP_SERVER_URL, headers=headers) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                result = await session.call_tool("get_my_loans", {})
...
@tool
async def get_loan_details_tool(loan_id: str, config: RunnableConfig) -> str:
    """
    ...
    try:
        ...
        async with sse_client(MCP_SERVER_URL, headers=headers) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                #call MCP tool by name and pass arguments
                result = await session.call_tool("get_loan_details", {"loan_id": loan_id})
